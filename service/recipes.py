...
from database.models import Recipes_db
from database import schemas
from service import hashtag
from utils.db import get_recipes, get_hashtags_by_tags, get_recipe_hashtag, get_top_likes, get_recipes_by_id, get_recipes_by_user, get_recipe_for_admin


def create_recipe(db: Session, recipe: schemas.RecipeCreate, author_id: int, tags: list) -> Recipes_db:
...
def add_photo(db: Session, recipe_id: int, url_photo: str) -> Recipes_db:
...
def get_recipe_by_filter(db: Session, name: str = None, type: str = None, tag: str = None) -> List[Recipes_db]:
    """Получите рецепты по фильтру. Если фильтры не установлены, возвращаются все рецепты"""
...
def get_top_recipe(db: Session, limit: int) -> List[models.Recipes_db]:
    """Получите лучшие рецепты"""
...
def get_user_recipes(db: Session, user_id: int) -> List[models.Recipes_db]:
    """Получите рецепты, принадлежащие пользователю"""
    return get_recipes_by_user(db, user_id)


def change_recipe(db: Session, recipe_id: int, recipe: schemas.RecipeChange, user_id: int) -> models.Recipes_db:
    """Изменить рецепт"""
...
def ban_recipe(db: Session, recipe_id: int) -> models.Recipes_db:
    """Скрыть рецепт от пользователей"""
