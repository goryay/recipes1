...
from service import user, recipes, hashtag, likes
...
class TestService:
    ...
    def test_create_recipe(self):
        db_recipe = recipes.create_recipe(self.db, self.new_recipe, self.user.id, self.tags)
    ...
    def test_get_recipe_by_filter(self, recipes=None):
        ...
        recipes.create_recipe(self.db, recipe_1, self.user.id, ['lemon', 'Chocolate'])
        recipe_2 = schemas.RecipeCreate(name='Vodka', description='Description',
                                        ...
                                        date_creation=datetime.today(), author_id=0)
        db_recipe = recipes.create_recipe(self.db, recipe_2, self.user.id, ['drink', 'alcohol'])
        recipes = recipes.get_recipe_by_filter(self.db)
        assert len(recipes) == 3
        recipes = recipes.get_recipe_by_filter(self.db, name='Recipe')
        assert len(recipes) == 1
        assert recipes[0].name == "Recipe"
        recipes = recipes.get_recipe_by_filter(self.db, type='Десерт')
        assert len(recipes) == 2
        recipes = recipes.get_recipe_by_filter(self.db, tag='drink')
        assert len(recipes) == 1
        assert recipes[0].name == "Vodka"
        recipes = recipes.get_recipe_by_filter(self.db, type='Десерт', tag='lemon')
        assert len(recipes) == 1
        assert recipes[0].name == 'Re 2'
        recipes = recipes.get_recipe_by_filter(self.db, name='re', tag='Chocolate')
        assert len(recipes) == 2
        recipes = recipes.get_recipe_by_filter(self.db, name='recipe', type='Десерт')
        assert len(recipes) == 1
        assert recipes[0].name == "Recipe"
        recipes = recipes.get_recipe_by_filter(self.db, name='re', type='Десерт', tag='Chocolate')
        assert len(recipes) == 2
    ...
    def test_delete_recipe(self):
        result = recipes.delete_recipe(self.db, self.recipe.id)
        assert result is True
        resipes_id = [db_recipe.id for db_recipe in recipes.get_recipes(self.db)]
